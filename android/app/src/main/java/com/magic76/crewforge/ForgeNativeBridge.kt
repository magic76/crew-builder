...
class ForgeNativeBridge(private val activity: Activity, private val onGeminiResult:(String)->Unit, private val onLiveEvent:(String)->Unit, private val onLocationResult:(String)->Unit) {
 companion object { private const val TAG="CrewBuilder"; private const val PREFS="crew_builder_config"; private const val LEGACY_PREFS="crew_forge_config"; private const val KEY_API_KEY="gemini_api_key"; private const val KEY_ALIAS="crew_builder_gemini_key"; private const val ENCRYPTED_PREFIX="enc:"; private const val MIC_REQUEST=701; const val LOCATION_REQUEST=702; private val FALLBACK_MODELS=listOf("gemini-3.6-flash","gemini-3.5-flash-lite","gemini-3.5-flash","gemini-3.0-flash","gemini-3-flash","gemini-2.5-flash","gemini-2.0-flash","gemini-1.5-flash") }
 ...
 fun vibrate(ms:Long){val d=ms.coerceIn(1,2000);val v=if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.S)activity.getSystemService(VibratorManager::class.java).defaultVibrator else activity.getSystemService(Vibrator::class.java);if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O)v.vibrate(VibrationEffect.createOneShot(d,VibrationEffect.DEFAULT_AMPLITUDE)) else @Suppress("DEPRECATION") v.vibrate(d)}
 fun share(title:String?,text:String?,url:String?){activity.runOnUiThread{val payload=listOfNotNull(text?.takeIf{it.isNotBlank()},url?.takeIf{it.isNotBlank()}).joinToString("\n");activity.startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply{type="text/plain";putExtra(Intent.EXTRA_SUBJECT,title.orEmpty());putExtra(Intent.EXTRA_TEXT,payload)},title?.takeIf{it.isNotBlank()}?:"Share"))}}
 fun copyToClipboard(text:String?){activity.runOnUiThread{(activity.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText("Crew Builder",text.orEmpty()))}}
 fun requestLocation(requestId:String){
  if(hasLocationPermission()){deliverLocation(requestId);return}
  pendingLocationRequests.add(requestId)
  if(locationPermissionInFlight)return
  locationPermissionInFlight=true
  activity.runOnUiThread{activity.requestPermissions(arrayOf(Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.ACCESS_COARSE_LOCATION),LOCATION_REQUEST)}
 }
 fun onLocationPermissionResult(granted:Boolean){
  locationPermissionInFlight=false
  val pending=pendingLocationRequests.toList();pendingLocationRequests.clear()
  pending.forEach{if(granted)deliverLocation(it) else deliverLocationResult(it,null,"Location permission was denied")}
 }
 private fun hasLocationPermission()=Build.VERSION.SDK_INT<23||activity.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)==PackageManager.PERMISSION_GRANTED||activity.checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)==PackageManager.PERMISSION_GRANTED
 private fun deliverLocation(requestId:String){
  try{
   val lm=activity.getSystemService(Context.LOCATION_SERVICE) as LocationManager
   val loc=listOf(LocationManager.GPS_PROVIDER,LocationManager.NETWORK_PROVIDER).mapNotNull{try{lm.getLastKnownLocation(it)}catch(_:Exception){null}}.maxByOrNull{it.time}
   if(loc==null){deliverLocationResult(requestId,null,"Location is not available yet");return}
   val value=JSONObject().put("lat",loc.latitude).put("lng",loc.longitude).put("accuracy",loc.accuracy).put("timestamp",loc.time).put("ageMs",(System.currentTimeMillis()-loc.time).coerceAtLeast(0))
   deliverLocationResult(requestId,value,null)
  }catch(e:Exception){deliverLocationResult(requestId,null,e.message?:"Location request failed")}
 }
 private fun deliverLocationResult(requestId:String,value:JSONObject?,error:String?){
  val payload=JSONObject().put("requestId",requestId).put("value",value?:JSONObject.NULL).put("error",error?:JSONObject.NULL).toString()
  activity.runOnUiThread{onLocationResult(payload)}
 }
 fun destroy(){pendingLocationRequests.clear();live.stop();executor.shutdownNow()}
}
