(() => {
  ...
  const optionLabels = {
    en: {
      ...
      liveStyleSelect: {
        concise: 'Concise · short and direct',
        balanced: 'Balanced · natural conversation',
      ...
      uiLanguageSelect: { en: 'English', 'zh-TW': 'Traditional Chinese' }
    },
    'zh-TW': {
      modelSelect: {
        auto: '自動 · 優先使用 3.6 Flash',
        'gemini-3.6-flash': 'Gemini 3.6 Flash',
        'gemini-3.5-flash': 'Gemini 3.5 Flash',
        'gemini-3.5-flash-lite': 'Gemini 3.5 Flash-Lite',
        'gemini-3.1-pro-preview': 'Gemini 3.1 Pro Preview'
      ...
      uiLanguageSelect: { en: '英文', 'zh-TW': '繁體中文' }
    }
  };

  function current() { return localStorage.getItem(KEY) || 'zh-TW'; }
  function localizeSelect(id, labels) {
    const node = document.getElementById(id);
    if (!node || !labels) return;
    [...node.options].forEach((option) => {
      if (labels[option.value] != null) option.textContent = labels[option.value];
    });
  }
  function apply(language = current()) {
    ...
    Object.entries(optionLabels[lang] || {}).forEach(([id, labels]) => localizeSelect(id, labels));
    ...
    if (lang === 'zh-TW') {
      ...
      if (apiKey && !apiKey.value) apiKey.placeholder = '貼上 API key';
    } else {
      ...
      if (apiKey && !apiKey.value) apiKey.placeholder = 'Paste API key';
    }
  }
